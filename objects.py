...
class Board:#Initializes the board with a list of images and creates an extended image list for shuffling.
	...
	def randomize_images(self):#Randomizes the images on the game board.
	...
	def info_cards(self):# Creates an information board with images.
...
class Card:
	def __init__(self, value, index, image, pos):#Initializes a card with a value, index, image, and position.
		self.value = value
		self.index = index
		self.image = image
		self.pos = pos
	...
	def on_click(self, win, speed=None):#Handles card animation and drawing when clicked.
		if self.visible:
			if self.slide_left:
				self.animation_complete = False
				if self.cover_x > 0:
...
class InfoCard:
	def __init__(self, value, index, image, pos):#nitializes an information card with a value, index, image, and position.
		self.value = value
		self.index = index
		self.image = image
		self.pos = pos
...
class Button(pygame.sprite.Sprite):
	def __init__(self, img, scale, x, y):#nitializes a button with an image, scale, and position.
	...
	def draw(self, win, image=None):#Draws the button and detects clicks.
		if image:
			self.image = image
		action = False
		...
		win.blit(self.image, self.rect)
		return action
#Displays a message box with text and a title using a specified font.
def message_box(win, font, name, text):
	...
	#Overall, this code demonstrates a structured and organized approach to creating a memory puzzle game using Pygame and OOP concepts.
